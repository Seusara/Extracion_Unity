...
def test_tutorial_stays_text_only_and_points_to_the_relevant_control() -> None:
    assert all("image" not in step and "caption" not in step for step in TUTORIAL_STEPS)
    assert all(step.get("where") for step in TUTORIAL_STEPS)
